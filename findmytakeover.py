...
import argparse
import sys  # Python 3.14 compatibility: Added sys module import for sys.exit() instead of exit()
import yaml
import click
import os
import pandas as pd
...
def readConfig(data):
    ...
    # Python 3.14 compatibility: Changed from open() without closing to context manager (with statement)
    # to ensure proper file handle cleanup and prevent resource leaks
    try:
        with open(data, "rt") as cf:
            config = yaml.safe_load(cf)
    except FileNotFoundError:
        # Python 3.14 compatibility: Added FileNotFoundError handling for better error reporting
        print(f"Configuration file not found: {data}")
        sys.exit(
            1
        )  # Python 3.14 compatibility: Changed from exit() to sys.exit() for proper script termination
    ...
    try:
        # Python 3.14 compatibility: Removed .keys() from dict membership check - more efficient and Pythonic
...
def main():
    ...
    for i in infra:
        if i == "aws":
            ...
            azureinfra = azure.infra(
                infra["azure"]["accounts"], infra["azure"]["credentials"]
            )
            for f in azureinfra:
    ...
    if dns != {} or infra != {}:
        result = pd.merge(
            records, infrastructure, left_on="dnsvalue", right_on="value", how="left"
        ).fillna(value="")

        # Checking for exclusions in the results
        for f in exclude:
            result = result[~result["dnsvalue"].str.contains(f, na=False)]
